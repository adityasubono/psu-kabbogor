$(document).ready(function() {
    // var a = 0;
    $("#add_data_softscape").click(function () {
        var jumlah = parseInt($("#jumlah-form").val()); // Ambil jumlah data form pada textbox jumlah-form
        var a = jumlah + 1; // Tambah 1 untuk jumlah form nya

        var pertamanan_id = $('#pertamanan_id').val();
        $("#softscape-form").append(
            '<div id="data_softscape_append" class="mt-3 rounded">'
            + '<h5><b>Data Softscape Ke-' + a + '</b></h5>'
            + '<hr class="bg-primary">'
            + '<div class="row my-3">'
            + '<div class="col-sm-3">'
            + '<input type="hidden" class="form-control" id="pertamanan_id"'
            + 'name="data_softscape['+ a +'][pertamanan_id]" value="'+ pertamanan_id +'">'

            + '<label for="nama_alat">Nama Alat</label><br>'
            + '<input type="text" class="form-control"'
            + 'id="nama_alat"name="data_softscape['+ a +'][nama_alat]"'
            ...
            + '<input type="text" name="data_softscape[' + a + '][tipe]" '
            ...
            + 'name="data_softscape['+ a +'][tahun_perolehan]" value="">'
            ...
            + 'id="kondisi" name="data_softscape['+ a +'][kondisi]">'
            ...
            + 'name="data_softscape['+ a +'][keterangan]" rows="3"'
    ...
    $(document).on('click', '#remove-data-softscape', function () {
        $(this).parents('#data_softscape_append').remove();
    });

    $("#btn-reset-form").click(function(){
        $("#softscape-form").html(""); // Kita kosongkan isi dari div
        // insert-form
