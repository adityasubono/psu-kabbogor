...
<textarea class="form-control" rows="5" disabled>
    @foreach( $koordinat_pertamanan as $koor ){{$koor->latitude}},{{$koor->longitude}},
    @endforeach</textarea>
            <div id="mymap"></div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        var locations_sarana = <?php print_r(json_encode($koordinat_pertamanan)) ?>;
        var mymap = new GMaps({
            el: '#mymap',
            lat: -6.485213,
            lng: 106.753537,
    ...
    <a href="/koordinatpertamanans/{{$koor->pertamanan_id}}"
