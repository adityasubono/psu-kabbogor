...
@section('title', 'Halaman Peta Sarana')
...
<textarea class="form-control" rows="5" disabled>
    @foreach( $koordinat_lengkap as $koor ){{$koor->latitude}},{{$koor->longitude}},
    @endforeach</textarea>
            <div id="mymap"></div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        var locations_sarana = <?php print_r(json_encode($koordinat_lengkap)) ?>;
        var mymap = new GMaps({
            el: '#mymap',
            lat: -6.485213,
            lng: 106.753537,
