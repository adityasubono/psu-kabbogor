...
<table class="table table-bordered table-hover display nowrap"
    ...
    <tr>
        ...
        <td>
            <button class="btn btn-warning btn-icon-split"
                    data-toggle="modal"
                    data-target="#edit_data_ippt{{ $ippt->id }}"
                    data-backdrop="static"
                    data-keyboard="false">
                <span class="icon text-white-50">
            ...
            <button class="btn btn-danger btn-icon-split"
                    data-toggle="modal"
                    data-target="#confirm_delete_data_ippt{{ $ippt->id }}"
                    data-backdrop="static"
                    data-keyboard="false">
                <span class="icon text-white-50">
    ...
    <div class="modal fade" id="edit_data_ippt{{ $ippt->id }}"
         tabindex="-1"
         role="dialog"
         aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @include('PSU_Perumahan.ippt.edit')
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="confirm_delete_data_ippt{{ $ippt->id }}"
         tabindex="-1"
         role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <b>Apakah Anda Akan Menghapus Data Ini ID {{$ippt->id}}?</b>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-success" data-dismiss="modal">
                        Batal
                    </button>
                    <form action="/ippt/delete/{{$ippt->id}}" method="post"
                          class="d-inline">
                        @method('delete')
                        <input type="hidden" name="perumahan_id"
                               value="{{$ippt->perumahan_id}}">
                        @csrf
